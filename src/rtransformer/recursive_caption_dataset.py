...
class RecursiveCaptionDataset(Dataset):
    ...
    def __init__(self, dset_name, data_dir, duration_file, word2idx_path,
        ...
        base_data = os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
        self.env_feature_dir = os.path.join(base_data, self.data_path[dset_name]["env_feat"])
        self.clip_feature_dir = os.path.join(base_data, self.data_path[dset_name]["clip_feat"])
        self.sent_feature_dir = os.path.join(base_data, self.data_path[dset_name]["sent_feat"])
    ...
    def fix_missing(self):
        ...
        for e in tqdm(self.data):
            ...
            feat_path_env = os.path.join(self.env_feature_dir, "v_{}.npy".format(video_name) if self.dset_name == "anet" else "{}.npy".format(video_name))
            ...
            for p in [feat_path_env, feat_path_clip]:
    ...
    def convert_example_to_features(self, example):
        ...
        feat_path_env = os.path.join(self.env_feature_dir, "v_{}.npy".format(video_name) if self.dset_name == "anet" else "{}.npy".format(video_name))
        feat_path_clip = os.path.join(self.clip_feature_dir, "v_{}.json".format(video_name) if self.dset_name == "anet" else "{}.json".format(video_name))
        video_feature = load_c3d_env(feat_path_env)
    ...
    def clip_sentence_to_feature(self, name, timestamp, sentence, video_feature, lang_features):
        ...
        # video + text tokens
        feat_video, video_tokens, video_mask, feat_lang, lang_mask = self._load_indexed_video_feature(
            video_feature, lang_features, timestamp, duration)
    ...
    def _load_indexed_video_feature(self, raw_vid_feat, raw_lang_feat, timestamp, duration):
        ...
        return feat_video, video_tokens, video_mask, feat_lang, lang_mask
