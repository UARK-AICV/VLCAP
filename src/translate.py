...
def run_translate(eval_data_loader, translator, opt):
    ...
    for raw_batch in tqdm(eval_data_loader, mininterval=2, desc="  - (Translate)"):
        if opt.recurrent:
            ...
            batched_data = [prepare_batch_inputs(step_data, device=translator.device)
            ...
            dec_seq_list = translator.translate_batch(
                model_inputs, use_beam=opt.use_beam, recurrent=True, untied=False, xl=opt.xl, mmt2=True)
...
def main():
    ...
    for eval_mode in opt.eval_splits:
        ...
        div_filepath = pred_file.replace(".json", "_div.json")
        eval_div_cmd = ["python", "evaluate-diversity.py", "-s", pred_file,
                        "-r", eval_references[0], "-o", div_filepath]
        # eval_div_cmd = ["python", "evaluateCaptionsDiversity.py", pred_file]
        subprocess.call(eval_div_cmd, cwd=opt.eval_tool_dir)
