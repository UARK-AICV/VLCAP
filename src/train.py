    ...
    for epoch_i in range(last_epoch, opt.n_epoch):
        ...
        if log_train_file and log_valid_file:
            with open(log_train_file, "a") as log_tf, open(log_valid_file, "a") as log_vf:
                ...
                # {loss: 8.5f},{ppl: 8.5f},{acc:3.3f}
                log_vf.write("{epoch},{m:.2f},{b:.2f},{c:.2f},{r:.2f}\n".format(
                    epoch=epoch_i, 
                    # loss=val_loss, ppl=math.exp(min(val_loss, 100)), acc=100*val_acc,
...
def get_args():
    ...
    parser.add_argument("--voc_path", type=str, default=None, help="extracted CLIP vectors")
    parser.add_argument("--freeze_voc", action="store_true", help="do not train CLIP vectors")
    ...
    if unknown:
        raise ValueError(f"unexpected arg: {unknown} is given")
